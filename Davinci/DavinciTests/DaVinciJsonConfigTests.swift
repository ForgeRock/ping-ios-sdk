...
final class DaVinciJsonConfigTests: XCTestCase, @unchecked Sendable {
    ...
    private func oidcClientConfig(of daVinci: DaVinci) -> OidcClientConfig? {
        daVinci.config.modules.first(where: { $0.config is OidcClientConfig })?.config as? OidcClientConfig
    }
    ...
    func testCreateDaVinci_success_openIdOnly_noDiscoveryEndpoint() {
        let result = DaVinci.createDaVinci(json: openIdOnlyJson)
        guard case .success(let daVinci) = result else {
            XCTFail("Expected success, got failure: \(result)"); return
        }
        guard let oidcClientConfig = oidcClientConfig(of: daVinci) else {
    ...
    func testCreateDaVinci_success_openIdOnly_lenient_tokenEndpointOnly() {
    ...
    func testCreateDaVinci_failure_openIdOnly_missingRequiredSubfield() {
        var json = openIdOnlyJson
        var oidc = json["oidc"] as! [String: Any]
        ...
        guard case .failure(let error) = result,
              case .missingRequiredField(let field) = error as? JsonConfigError else {
            XCTFail("Expected missingRequiredField(oidc.openId.tokenEndpoint), got: \(result)")
            return
        }
        XCTAssertEqual(field, "oidc.openId.tokenEndpoint")
    }

    func testCreateDaVinci_success_discoveryEndpointAndOpenIdOverride_unchanged() {
        var json = minimalJson
        var oidc = json["oidc"] as! [String: Any]
        oidc["openId"] = [
