...
`openIdOverride` still runs on top of whichever document is used — discovered or pre-supplied:
...
`oidc.discoveryEndpoint` is required *unless* an `oidc.openId` sub-object is supplied. When `openId` is supplied **without** `discoveryEndpoint`, it replaces the discovery document (no network call) and `oidc.openId.tokenEndpoint` becomes required. Any other non-optional endpoint you omit defaults to an empty string, so supply every endpoint your flow actually uses.
